...
function TablaTransporte() {
  ...
  const [idTransporte, setIdTransporte] = useState(0.0);
      ...
      fn: (items) => {
        if (target.value === "") return items;
          ...
          return items.filter((x) => {
            if (
              (x.transporte_id !== null
                ? String(x.transporte_id).includes(target.value)
                : 0) ||
              (x.transporte_nombre !== null
                ? x.transporte_nombre
                    .toLowerCase()
              ...
              recordsAfterPagingAndSorting().map((item, index) => (
                <TableRow key={index} style={{ padding: "0" }}>
                  ...
                  <TableCell
                    ...
                    style={{ textAlign: "center", alignItems: "center" }}
                  >
                    {parseInt(item.transporte_estatus) === 1
                      ? "ACTIVO"
                      : "INACTIVO"}
                  </TableCell>
                  ...
                  >
                    <button
                      onClick={gestionarBanco(2, item.transporte_id)}
                      className="btn btn-sm mx-1 btn-warning rounded-circle"
                    >
                      <i className="fa fa-edit"></i>{" "}
                    </button>
                    <button
                      onClick={gestionarBanco(3, item.transporte_id)}
