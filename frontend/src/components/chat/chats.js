...
{
  Array.isArray(conversacion) && conversacion.length > 0 && conversacion.map((item, index) => {
    if (item.remitente !== user_id) {
      return (
        <div className="d-flex justify-content-start mb-4" key={index}>
          <div className="msg_cotainer">
            {item.content_sms}
            <span className="msg_time">{item.fecha_hora_sms}</span>
          </div>
        </div>
      );
    } else {
      return (
        <div className="d-flex justify-content-end mb-4" key={index}>
          <div className="msg_cotainer_send">
            {item.content_sms}
            <span className="msg_time_send">{item.fecha_hora_sms}</span>
          </div>
        </div>
      );
    }
